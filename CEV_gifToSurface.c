...
void gif_getSize(CEV_GifAnim *gif, int *width, int *height) {
    *width = gif->display.pos.w;
    *height = gif->display.pos.h;
}
...
{//redraw method, anything off the frame rect is made full alpha
    ...
    //TODO (cev#1#): could be done linearly with 1 loop, but not so understandable, W8&C if slow
    for(y =0; y < height; y++)
    {
        for(x =0; x < width; x++)
        {
...
{//overlay method, only frame rect is copied to surface
    ...
    void* access;
    //TODO (cev#1#): Error handeling here
